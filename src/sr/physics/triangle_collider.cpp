...
static sr::math::Vec3 closest_point_on_triangle(const sr::math::Vec3& p,
                                                const sr::math::Vec3& a,
                                                const sr::math::Vec3& b,
                                                const sr::math::Vec3& c) {
  ...
  for (int cz = a.z; cz <= b.z; ++cz) {
    for (int cx = a.x; cx <= b.x; ++cx) {
      ...
      for (uint32_t ti : it->second) {
        if (ti >= seen_stamp_.size()) continue;
        if (seen_stamp_[ti] == stamp) continue;
...
Contact TriangleMeshCollider::resolve_sphere(sr::math::Vec3& center, float radius,
  ...
  Contact res;
  if (tris_.empty() || radius <= 0.0f) return res;
  ...
  for (int it = 0; it < std::max(1, iterations); ++it) {
    ...
    for (uint32_t ti : cands) {
      ...
      // Quick XZ reject.
      if (center.x + radius < t.minx || center.x - radius > t.maxx) continue;
      if (center.z + radius < t.minz || center.z - radius > t.maxz) continue;
...
RayHit TriangleMeshCollider::raycast_down(float x, float z, float y_start, float max_dist) const {
  RayHit best;
  if (tris_.empty() || max_dist <= 0.0f) return best;
  ...
  for (uint32_t ti : cands) {
    ...
    if (tval < 0.0f || tval > max_dist) continue;
    ...
    if (sr::math::dot(sr::math::cross(ab, ap), t.n) < 0.0f) continue;
    if (sr::math::dot(sr::math::cross(bc, bp), t.n) < 0.0f) continue;
    if (sr::math::dot(sr::math::cross(ca, cp), t.n) < 0.0f) continue;
