...
void FlyCamera::apply_keys(const uint8_t* keys, float dt) {
  ...
  if (keys[SDL_SCANCODE_SPACE]) move = move + world_up;
  if (keys[SDL_SCANCODE_LSHIFT] || keys[SDL_SCANCODE_RSHIFT]) move = move - world_up;

  float mag = sr::math::length(move);
  if (mag > 0.0f) move = move / mag;

  float speed = move_speed;
  if (keys[SDL_SCANCODE_LCTRL] || keys[SDL_SCANCODE_RCTRL]) speed *= fast_mul;
