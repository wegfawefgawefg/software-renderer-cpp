...
sr::gfx::Texture load_texture_rgba8888(SDL_Renderer* renderer, const std::string& path) {
  ...
  if (!surf) throw std::runtime_error(std::string("IMG_Load failed: ") + IMG_GetError());
  ...
  if (!conv) throw std::runtime_error(std::string("SDL_ConvertSurfaceFormat failed: ") + SDL_GetError());
  ...
  for (int y = 0; y < h; ++y) {
    ...
    for (int x = 0; x < w; ++x) {
      pixels[size_t(y) * size_t(w) + size_t(x)] = row[x];
    }
  }

  SDL_FreeSurface(conv);
  return sr::gfx::Texture(w, h, std::move(pixels));
}
