...
Model load_obj_model(const std::filesystem::path& path, AssetStore& store,
  ...
  std::ifstream f(path);
  if (!f) throw std::runtime_error("failed to open obj: " + path.string());
      ...
      if (!mtl_name.empty()) {
        ...
        if (std::filesystem::exists(mtl_path)) {
          ...
          for (auto& m : mats) {
            ...
            if (mat_index.find(m.name) != mat_index.end()) continue;
  ...
  model.mesh.indices.reserve([&]() {
    size_t n = 0;
    for (const auto& kv : mat_indices) n += kv.second.size();
    return n;
  }());

  for (uint32_t mi = 0; mi < model.materials.size(); ++mi) {
    auto it = mat_indices.find(mi);
    if (it == mat_indices.end() || it->second.empty()) continue;
    Primitive prim;
